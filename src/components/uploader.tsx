...
const uploader = async() =>{
    ...
    try {
        const response = await uploadDocumentService({album:album,files:files});
        if(response?.status === 200)
        swal("Good job!", "Photo(s) uploaded successfully",{className:"swal"}).then(()=>{
...
const eventHandlers = { addedfile: (file:File) => {
    let temp = files;
    temp.push(file);
    setFiles(temp)} }
  ...
  return (
    <>
      ...
      >
        <ModalHeader className="modal-bg" toggle={()=>toggle(false)} >Upload Photos</ModalHeader>
        ...
        djsConfig={djsConfig} >
            <div className="text-center child">
                <CloudUploadIcon />
            <p>Drag and drop an image or click here to select</p></div>
        </DropzoneComponent>
        <div className="form-group mt-3">
        <label >Album</label>
        <select className="form-control" id="album" onChange={(e)=>setAlbum(e.target.value)}>
        ...
        <ModalFooter className="modal-bg" >
            <div className="" >
                    <Button color="primary" onClick={uploader}><CloudUploadIcon />&nbsp;upload</Button>
